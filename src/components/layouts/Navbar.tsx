      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-bold text-brand-orange text-xs tracking-widest leading-none mt-1">PRESTASI PRIMA</span>
          </div>
        </Link>
          ...
          {MENU.map((menu) => (
            ...
            >
              <button className="px-5 py-2.5 rounded-full text-[11px] font-black uppercase tracking-widest text-white/80 hover:text-white hover:bg-white/10 transition-all flex items-center gap-2 group">
                {menu.title}
                <RiArrowDownSLine className={`text-lg transition-transform duration-300 ${hoveredMenu === menu.id ? "rotate-180 text-brand-orange" : ""}`} />
              </button>
              ...
              <AnimatePresence>
                {hoveredMenu === menu.id && (
                  <motion.div
                    ...
                    className="absolute left-0 mt-3 w-72 bg-brand-dark/95 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden"
                  >
                    {/* Decorative Background for Dropdown */}
                    <div className="absolute top-0 right-0 w-32 h-32 bg-brand-orange/10 blur-[40px] rounded-full -mr-16 -mt-16 pointer-events-none" />
                    ...
                    <div className="relative z-10 space-y-1">
                      {menu.items?.map((item) => (
                        ...
                        >
                          <span className="text-sm font-bold text-white/70 group-hover/item:text-brand-orange">{item.label}</span>
                          <RiArrowRightSLine className="text-white/20 group-hover/item:text-brand-orange transition-colors" />
                        </Link>
                      ))}

                      {menu.subgroups?.map((sub) => (
                        <div key={sub.title} className="pt-4 mt-4 border-t border-white/5">
                           <h4 className="px-5 text-[9px] font-black uppercase tracking-[0.3em] text-brand-orange mb-2">{sub.title}</h4>
                           {sub.items.map((child) => (
                             <Link
                               key={child.label}
                               href={child.url}
